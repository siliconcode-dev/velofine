...
**Status:** `1.8-Beta`. v1 (all nine build phases — see `Build_plan_v1.5.md`'s history section for the original phased plan) shipped without a confirmed fix for the flagship Intel Gen7 rendering bug; v1.5 added targeted shader patches plus a standalone `diagnostic.exe` tool, gated behind an opt-in "Experimental Legacy Fix" toggle. v1.6-Beta and v1.7-Beta *attempted* two targeted fixes — Mojang's MC-308593 animated-texture regression (invisible water, black lava/fire/nether portal) and the invisible end portal — but real tester logs confirmed **neither ever executed**: one threw on every call, the other bailed out, and on a clean config detection never enabled either. v1.8-Beta fixes those four root causes (a compile-time-constant inlining trap, an interception stage that ran before Minecraft injected the shader's `#define`s, hardware detection that couldn't match the real reference machine's WMI output, and an auto-updater that never followed GitHub's redirects). Both targeted fixes remain gated to hardware personally verified to hit these bugs (`GpuConfidence.EXACT_VERIFIED`), and **neither has yet been observed working on real hardware** — that confirmation is the outstanding acceptance test. See `CLAUDE.md` for exactly which claims are and aren't confirmed.
