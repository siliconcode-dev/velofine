...
**Status:** `1.7-Beta`. v1 (all nine build phases — see `Build_plan_v1.5.md`'s history section for the original phased plan) shipped without a confirmed fix for the flagship Intel Gen7 rendering bug; v1.5 added targeted shader patches plus a standalone `diagnostic.exe` tool, gated behind an opt-in "Experimental Legacy Fix" toggle. v1.6-Beta fixed Mojang's own MC-308593 regression (invisible water, black lava/fire). v1.7-Beta adds a fix for the invisible end portal — its fragment shader indexes a color array with a runtime loop variable, a known weak point for older Intel shader compilers; the loop is unrolled into static indices instead. Both targeted fixes are gated to hardware personally verified to hit these bugs (`GpuConfidence.EXACT_VERIFIED`), not the broader Intel Gen7 family. Some LegacySupport/Optimus findings remain research-grounded and bytecode-verified but **not yet confirmed on the actual reference hardware** — see `CLAUDE.md` for exactly which claims are and aren't confirmed.
